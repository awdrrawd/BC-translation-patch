...
for (const file of walk(root, f => /\.(ts|vue)$/.test(f))) {
    ...
    if (file.includes(`${path.sep}rules${path.sep}`)) {
...
const candidates = [...entries].sort(([a], [b]) => a.localeCompare(b)).map(([text, files]) => ({ text, files: [...files], translated: Object.hasOwn(dictionary, text) }));
const commit = execFileSync("git", ["-C", root, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
const report = { commit, note: "Static candidates only, not UI coverage. Dynamic expressions and standalone dialogs need manual review.", candidates };
